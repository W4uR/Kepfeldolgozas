...
class Old
{
	...
	*/
	...
	void static vilagosit1(Mat source) { //Nem kell referencia mert a "Mat" csak egy fejléc az adatrész csak egy mutató
		source += 50;
	}
	void static vilagosit1(Mat source, Mat& dest) {
		dest = source + 50;
	}

	void static invertalas(Mat source, Mat& dest) {
		dest = 255 - source;
	}

	void static narancsMasolo(const cv::Mat src, cv::Mat& dest) {
	...
	void static eloterMasolo(const cv::Mat fg, cv::Mat& bg) {
	...
	void static createHisto(const Mat img, Mat& hiszto) {
	...
	int static calc_th_value(const Mat src, float ratio = 0.1f) {
